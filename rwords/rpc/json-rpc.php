...
function handle_json_rpc($object) {
    try {
        ...
        if ($method !== "login") {
            if (!preg_match('/^\/([a-zA-Z0-9]+)/', $method, $match)) {
                throw new JsonRpcExeption(103,  "Invalid Request: no method $method");
            }
            else {
                $method = $match[1];
            }
        ...
        if (strcmp($method, "system.describe") == 0) {
            ...
            if (in_array("__call", $methods) && !in_array($method, $methods)) {
                $result = call_user_func_array(array($object, $method), array($params));
                ...
                //if ($num_got != 2) {
                //    $msg = "Wrong number of parameters. Got $num_got expect 1";
